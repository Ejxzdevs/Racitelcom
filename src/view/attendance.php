    ...
    <div class="overflow-x-auto" >
      ...
      <tbody>
        ...
        <tr class="border-b hover:bg-gray-50 transition-colors">
            ...
            <td class="py-2 text-center"><?php echo htmlspecialchars($display['attendance_name']); ?></td>
...
<div id="modal" class="fixed inset-0 bg-opacity-50 flex justify-center items-center hidden" style="background-color: rgba(0, 0, 0, 0.5);">
  <div class="bg-white p-6 rounded-lg w-96 max-h-[80vh] overflow-y-auto">
    <h2 class="text-[20px] mb-4">New attendance</h2>
    <form id="addForm" action="../controller/attendanceController.php" method="POST">
      <!-- Input Field -->
      <div class="mb-4">
        <input type="hidden" name="csrf_token" value="<?php echo $csrf_token ?>">
        <input type="hidden" name="insert">
        <label for="attendance_name" class="block text-[14px]  font-medium text-gray-700">attendance Name</label>
        <input type="text" id="attendance_name" name="attendance_name" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter attendance Name:" required>
      </div>
      <div class="mb-4">
...
<div id="editmodal" class="fixed inset-0 bg-opacity-50 flex justify-center items-center hidden" style="background-color: rgba(0, 0, 0, 0.5);">
  <div class="bg-white p-6 rounded-lg w-96 max-h-[80vh] overflow-y-auto">
    <h2 class="text-[20px] mb-4">Update attendance</h2>
    <form id="editForm" action="../controller/attendanceController.php" method="POST">
      <!-- Input Field -->
      <div class="mb-4">
        <input type="hidden" name="csrf_token" value="<?php echo $csrf_token ?>">
        <input type="hidden" name="update">
        <input type="hidden" id="edit_attendance_id" name="attendance_id" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter attendance Id:" required>
        <label for="attendance_name" class="block text-sm font-medium text-gray-700">attendance Name</label>
        <input type="text" id="edit_attendance_name" name="attendance_name" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter attendance Name:" required>
      </div>
      <div class="mb-4">
        <label for="time_start" class="block text-sm font-medium text-gray-700">Time Start</label>
