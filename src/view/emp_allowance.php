<?php 
require_once '../services/emp_allowanceApi.php';
require_once '../services/allowanceApi.php';
require_once '../helper/csrf.php';
$csrf_token = CsrfHelper::generateToken();
$getData = new emp_allowanceApi();
$data = $getData->getAll();

$getallowance = new AllowanceApi();
$allowance = $getallowance->getAll();
?>
    ...
    <div class="overflow-x-auto" >
      <table class="w-full table-fixed rounded-lg bg-white shadow-md overflow-hidden" >
        <thead class="bg-gray-200">
          <tr class="text-gray-700 border-b">
            ...
            <th class="py-2 text-center">Allowance ID</th>
      ...
      <tbody>
        ...
        <tr class="border-b hover:bg-gray-50 transition-colors">
            <td class="py-2 text-center"><?php echo htmlspecialchars($display['emp_allowance_id']); ?></td>
            <td class="py-2 text-center"><?php echo htmlspecialchars($display['employee_id']); ?></td>
            <td class="py-2 text-center"><?php echo htmlspecialchars($display['allowance_id']); ?></td>
            <td class="py-2 text-center">
                <?php echo htmlspecialchars(date('M d Y ', strtotime($display['created_at']))); ?>
            </td>
            <td class="py-2 text-center"><?php echo htmlspecialchars($display['emp_allowance_status']); ?></td>
            <td class="py-2 flex flex-row justify-center gap-4">
              <form action="../controller/emp_allowanceController.php" method="POST">
                <input type="hidden" name="csrf_token" value="<?php echo $csrf_token ?>">
                <input type="hidden" name="delete">
                <input type="text" name="id" value="<?php echo htmlspecialchars( $display['emp_allowance_id'] );?>" hidden>
...
<div id="modal" class="fixed inset-0 bg-opacity-50 flex justify-center items-center hidden" style="background-color: rgba(0, 0, 0, 0.5);">
  <div class="bg-white p-6 rounded-lg w-96 max-h-[80vh] overflow-y-auto">
    <h2 class="text-[20px] mb-4">Employee Allowance</h2>
    <form id="addForm" action="../controller/emp_allowanceController.php" method="POST">
      <!-- Input Field -->
      <div class="mb-4">
        <input type="hidden" name="csrf_token" value="<?php echo $csrf_token ?>">
      ...
      <div class="mb-4">
        <label for="allowance_id" class="block text-sm font-medium text-gray-700">Allowance</label>
          <select id="allowance_id" name="allowance_id" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            <option value="" disabled selected>Choose Allowance</option>
              <?php foreach($allowance as $data): ?>
                <option value="<?php echo $data['allowance_id'] ?>"><?php echo $data['allowance_name'] ?></option>
              <?php endforeach; ?>
      </select>
      </div>
      <!-- Submit Button -->
...
<div id="editmodal" class="fixed inset-0 bg-opacity-50 flex justify-center items-center hidden" style="background-color: rgba(0, 0, 0, 0.5);">
  <div class="bg-white p-6 rounded-lg w-96 max-h-[80vh] overflow-y-auto">
    <h2 class="text-[20px] mb-4">Update emp_allowance</h2>
    <form id="editForm" action="../controller/emp_allowanceController.php" method="POST">
      <!-- Input Field -->
      <div class="mb-4">
        <input type="hidden" name="csrf_token" value="<?php echo $csrf_token ?>">
        <input type="hidden" name="update">
        <input type="hidden" id="id" name="emp_allowance_id" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
      ...
      <div class="mb-4">
        <label for="edit_allowance_id" class="block text-sm font-medium text-gray-700">Allowance</label>
          <select id="edit_allowance_id" name="allowance_id" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            <option value="" disabled selected>Choose Allowance</option>
              <?php foreach($allowance as $data): ?>
                <option value="<?php echo $data['allowance_id'] ?>"><?php echo $data['allowance_name'] ?></option>
              <?php endforeach; ?>
      ...
      <div class="mb-4">
        <label for="edit_emp_allowance_status" class="block text-sm font-medium text-gray-700">Status</label>
        <select id="edit_emp_allowance_status" name="emp_allowance_status" class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
...
<script src="../assets/js/emp_allowance.js"></script>